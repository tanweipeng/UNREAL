...
public class DatabaseHandler : MonoBehaviour
{
    ...
    //change maxScore to number of users
    ...
    //will be overwrite soon
    private int score = 100;
    //private bool addScorePressed;

    const int kMaxLogSize = 16382;
    //seems assuming that "One or more required services is unavailable for an unknown reason." so used UnavailableOther
    ...
    void Start()
    {
        ...
        FirebaseApp.CheckAndFixDependenciesAsync().ContinueWith(task =>
        {
            ...
            {
                Console.WriteLine("The DependencyStatus is Available");
                InitializeFirebase();
            }
            else
            {
    ...
    TransactionResult AddScoreTransaction(MutableData mutableData)
    {
        ...
        //create dict every time a min value removed???
        // Now we add the new score as a new entry that contains the email address and score.
        Dictionary<string, object> newScoreMap = new Dictionary<string, object>();
        newScoreMap["score"] = score;
        newScoreMap["email"] = Username;
